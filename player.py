...
class VinPlayer():
    ...
    def __init__(self, checkpoint_path="vindinium_network"):
        ...
        self._input_layer, self._output_layer = VinPlayer._create_network()
    ...
    def _create_network():
        # network weights
        convolution_weights_1 = tf.Variable(tf.truncated_normal([8, 8, VinPlayer.STATE_FRAMES, 32], stddev=0.01))
        ...
        feed_forward_weights_2 = tf.Variable(tf.truncated_normal([256, VinPlayer.ACTIONS_COUNT], stddev=0.01))
        feed_forward_bias_2 = tf.Variable(tf.constant(0.01, shape=[VinPlayer.ACTIONS_COUNT]))

        input_layer = tf.placeholder("float", [None, VinPlayer.SCREEN_Y + 4, VinPlayer.SCREEN_X, VinPlayer.STATE_FRAMES])
    ...
    def _parse_move_as(self, move, player):
        ...
        state = np.full((VinPlayer.SCREEN_Y + 4, VinPlayer.SCREEN_X), 1.)
        hero_index = 1
